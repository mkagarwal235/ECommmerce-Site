...
const Chechout = ({ card, addToCard, removeFromCard, subTotal,clearCard}) => {
  ...
  return (
    <div className='container px-2 sm:m-auto'>
      ...
      <div className="sideCard bg-blue-100 m-4 p-8">
          ...
          {Object.keys(card).map((k) => {
            return <li key={k}>
              <div className="item flex my-4">
                ...
                <div className='flex items-center justify-center w-1/3 font-semibold text-lg'><AiFillMinusCircle onClick={() => { removeFromCard(k, 1, card[k].price, card[k].name, card[k].size, card[k].variant) }} className='cursor-pointer text-blue-500' />
                  <span className='mx-2 
      ...
      <div className="m-4">
        <Link href={'/chechout'}><button disabled={disabled} onClick={initiatePayment} className="disabled:bg-blue-300 flex   mr-2 text-white bg-blue-500 border-0 py-2 px-2 
                focus:outline-none hover:bg-blue-600 rounded text-sm"><BsFillBagCheckFill className='m-1' />Pay ₹{subTotal}</button></Link>
      </div>
