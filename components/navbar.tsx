...
const Navbar = ({ user, logout, card, addToCard, removeFromCard, clearCard, subTotal }) => {
    ...
    return (
        <>
            {!sidebar && <span className='absolute right-12 top-3 z-30 cursor-pointer' onMouseOver={() => setdropdown(true)} onMouseLeave={() => setdropdown(false)} >
            ...
            <div className={`flex flex-col md:flex-row md:justify-start justify-center items-center py-2 mb-1 shadow-md sticky top-0 bg-white z-10 ${!sidebar && 'overflow-hidden'}`}>
                ...
                <div ref={ref} className={`w-72 h-[100vh] sideCard overflow-y-scroll absolute top-0  bg-pink-100 py-10 px-8  transition-all ${sidebar ? 'right-0' : '-right-96'}`}>
